...
velocityIterationList = []
yawIterationList = []
omegaIterationList = []

for i in range(0,len(velocityList)):
	for j in range(0,len(yawList)):
		velocityIterationList.append(velocityList[i])
...
def createRobotCommand(robot_id, test, tick_counter, period_fraction, t_test_start):
	...
	if test == "constant-angular-velocity-range":
		...
		# Check if still within experiment time
		if periodsPassed < len(angularVelocityList):
			# Set angular velocity of robot
			cmd.angularVelocity = angularVelocityList[math.floor(periodsPassed)]
