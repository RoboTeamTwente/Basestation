...
uint32_t drawRobotInfo(uint8_t id, bool USBstatus){
	...
	/* CALCULATE DATA */
	int16_t angle				= (msgBuff[(int)id].msg[2] & 0b00000011) << 8;
	angle |= msgBuff[(int)id].msg[3] & 0b11111111;
	angle |= (angle & 0x0200) ? 0xFC00 : 0x0000; // sign extension
	char angle_char[5];
	sprintf(angle_char, "%.2f", (float)angle);

	uint16_t theta				= (msgBuff[(int)id].msg[1] & 0b00011111) << 6;
	theta |= (msgBuff[(int)id].msg[2] >> 2) & 0b00111111;
	theta |= (theta & 0x0400) ? 0xF800 : 0x0000; // sign extension

	uint16_t rho 				= msgBuff[(int)id].msg[0] << 3;
	rho |= (msgBuff[(int)id].msg[1] >> 5) & 0b00000111;
	...
	bool kickStatus 			= (msgBuff[(int)id].msg[5] & 0b10000000) >> 7;
	bool chipStatus 			= (msgBuff[(int)id].msg[5] & 0b01000000) >> 6;
	bool dribblerStatus;
	if (((msgBuff[(int)id].msg[6] & 0b11111000) >> 3) > 0){
		dribblerStatus = true;
	} else {
		dribblerStatus = false;
	}
