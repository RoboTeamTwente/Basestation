...
def create_robot_command(test: str, robot_id: int, tick_number: int) -> REM_RobotCommand:
	"""
	...
	#test order: forward > backward > left > right > spin-left > spin-right

	int start_tick_number = tick_number
	int elapsed_ticks = tick_number - start_tick_number

	cmd.dribblerOn = 0

	if elapsed_ticks <= 120: #forward
		cmd.theta = 0
		cmd.rho = 0.3 - 0.3 * math.cos( 4 * math.pi * tick_number / 120 )
	elif elapsed_ticks > 120 and <= 240: #backward
		cmd.theta = -math.pi
		cmd.rho = 0.3 - 0.3 * math.cos( 4 * math.pi * tick_number / 120 )
	elif elapsed_ticks > 240 and <= 360: #left
		cmd.theta = -math.pi/2
		cmd.rho = 0.3 - 0.3 * math.cos( 4 * math.pi * tick_number / 120 )
	elif elapsed_ticks > 360 and <= 480: #right
		cmd.theta = math.pi/2
		cmd.rho = 0.3 - 0.3 * math.cos( 4 * math.pi * tick_number / 120 )
	elif elapsed_ticks > 480 and <= 600:
		cmd.yaw = -math.pi + math.pi/2 * (int(tick_number / 30) % 4)
	elif elapsed_ticks > 600 and <= 720:
		cmd.yaw = -math.pi + math.pi/2 * (int(tick_number / 30) % 4)
...
def main() -> None:
	...
	while True:
		...
		if time_till_next_tick < 0:
			...
			tick_number += 1
			cmd = create_robot_command(args.test, robot_id, tick_number)
