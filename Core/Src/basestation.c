...
void handlePIDCommand(uint8_t* packet_buffer){
  handled_PIDCommand++;

  // Check if the packet REM version corresponds to the local REM version. If the REM versions do not correspond, drop the packet.
  uint8_t packet_rem_version = REM_RobotCommand_get_remVersion((REM_PIDConfigurationPayload*) packet_buffer);
  if(packet_rem_version != LOCAL_REM_VERSION){
    sprintf(logBuffer, "[handlePIDCommand] Error! packet_rem_version %u != %u LOCAL_REM_VERSION.", packet_rem_version, LOCAL_REM_VERSION);
    return;
  }

  // Store the message in the RobotCommand buffer. Set flag indicating packet needs to be sent to the robot
  uint8_t robot_id = REM_RobotCommand_get_id((REM_PIDConfigurationPayload*) packet_buffer);
  memcpy(buffer_PIDCommand[robot_id].packet.payload, packet_buffer, PACKET_SIZE_REM_P_I_D_CONFIGURATION);
  buffer_PIDCommand[robot_id].isNewPacket = true;
  buffer_PIDCommand[robot_id].counter++;
}
...
bool handlePacket(uint8_t* packet_buffer, uint32_t packet_length){
  ...
  while(bytes_processed < packet_length){
    ...
    switch (packet_type){
      ...
      case PACKET_TYPE_REM_P_I_D_CONFIGURATION:
        handlePIDCommand(packet_buffer + bytes_processed);
        bytes_processed += PACKET_SIZE_REM_P_I_D_CONFIGURATION;
        break;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
  ...
  * exceed more than 127 bytes, per the SX1280 datasheet and our settings. */
  ...
  if(htim->Instance == htim1.Instance){
    ...
    && total_packet_length + PACKET_SIZE_REM_P_I_D_CONFIGURATION < MAX_PACKET_SIZE){
      buffer_PIDCommand[idCounter].isNewPacket = false;
      memcpy(sendBuffer + total_packet_length, buffer_PIDCommand[idCounter].packet.payload, PACKET_SIZE_REM_P_I_D_CONFIGURATION);
      total_packet_length += PACKET_SIZE_REM_P_I_D_CONFIGURATION;
    }
